...
`SERVICE_TOKEN` for its internal route. Monster Raid requires `raidUserToken` and
`raidAdminToken` to match identities configured through `RAID_AUTH_TOKENS`. The admin identity must
include the `admin` role.

For Monster Raid, choose your own local token mappings instead of committing them:
